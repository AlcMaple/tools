...
| **上报**（页面 SDK 主动 capture） | 失败诊断，或需要浏览器 / 设备上下文的事件 | 不需要上下文的普通日志不开 issue，避免占配额 |
...
Sentry 看到的会是 Node / Ubuntu 上下文，即使请求里带了 User-Agent，也不会因为它被放在
`extra` 里就自动解析成 browser / os / device。
...
5. 再打开一次该页面，触发一条**普通且不需要设备上下文的日志**，确认它**没有**在浏览器项目里开 issue，
