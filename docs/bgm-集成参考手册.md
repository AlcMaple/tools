...
- ❌ **应用层自动重试**（withRateLimitRetry / 5xx retry）—— 限流期间自动重试**加重惩罚窗口**。算过账：1 + 网络层 retry + 5xx retry + 限流 retry = 最坏 8 次。已经全部撤掉，**不要再加**
...
| HTTP 429 | 看 `Retry-After` 头 | 等倒计时归零再操作 |
| HTTP 5xx | 浏览器开 bgm.tv 能登录吗？不能 → 站点故障 | 等 BGM 自己恢复，跟我们无关 |
| HTTP 200 + 中文限流页 | search.ts 的 Layer 3 检测会抓到 | 倒计时 + 自然等 |
...
| **BGM 搜索** | **14 天** | **SWR**：先显示旧结果，后台静默刷缓存，下次搜才看到新 |
| **BGM 详情** | **永久** | 命中就用（官方元数据几乎不变） |
| **BGM 周历** | **14 天** | `update=true` 强制刷新，失败抛错让 UI 看到（不静默 fallback） |
