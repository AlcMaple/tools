...
## 2026-08-21 feat(web): 预转改为边转边播（EVENT playlist）

**效果**：

1. 遇到只有慢源的番，**开播等待从 12.5 分钟降到约 1 分钟**——不必等整集转完。
   本机实测：60 秒时已转出 24 片（144 秒内容），判定可播。

**为什么单靠 12 路并发救不了慢源**（这是走到这一步的前提）：

```text
入口  源站 → 服务器   12 路并发 5.0~5.3Mbps       ✓ 够
出口  服务器 → 手机   <video> 只开**一条**连接
                      跨境单连接 2.2Mbps < 2.67    ✗ 卡
```

即使只有一个人看、6Mbps 全给他也没用：瓶颈是**单连接限速**而不是总带宽，剩下的
带宽他没有第二条连接去用。只有分片才能让播放器开多连接——这就是必须走 HLS 的原因。

**边转边播怎么成立的**：

```text
-hls_playlist_type event   playlist 边转边追加，不必等 ENDLIST
攒够 15 片(90 秒内容)      放行开播，别让播放进度贴着转码前沿跑
remux 1.3x~2.4x 实时       领先量持续拉大，看完时也转完了
ffmpeg 正常结束            自动补 #EXT-X-ENDLIST → 就地变成完整 VOD，可随意 seek
```

**和让位逻辑天然兼容**：用户看 HLS 是读本地磁盘、不占入口带宽，所以 remux **不会**
被冻结（已实测：看 HLS 时预转持续推进 0→15→21MB，无暂停）。只有看未预转的 mp4 代理
才会抢入口，那时才该让位。

**状态条文案**沿用播放页的描稿隐喻：起稿中 → 边描边看 → 已上色完毕。

## 2026-08-21 feat(web): 默认挑直连线路 + 代理观看名额闸门

**效果**：

1. 打开播放页默认播**不需要代理的那条线路**，服务器带宽只留给真正需要它的源。
2. 代理名额满时，第三个人看到排队提示而不是三个人一起卡。

**一、默认线路按域名挑，不是按线路编号**

线路号和源站**没有固定对应关系**——实测 `animeId=3498` 的「线路一」就是
`play.xfvod.pro`（直连快源），而 3535 的线路一才是 `apn.moedot.net`（慢源）。
所以「默认用线路二」这个说法本身不成立，判据只能是 hostname。

```text
抓 source 1（原有的一次请求）
├─ 它不需要代理  → 直接用，**不多打任何请求**（懒加载原意不变）
└─ 它需要代理    → 才多花一次请求看看下一条是不是直连；是就换，不是就老实用它
```

白名单挪到 `proxy-hosts.ts` 单独成文件：解析层和传输层都要用，而 resolve.ts 不该
为一个常量把 undici 那一坨拖进 import 图。

**二、名额闸门**

出口 6Mbps ÷ 2.67Mbps ≈ **2.2 人**。第三个人挤进来不是「他自己卡」——三条连接公平
分带宽，每人只剩 2Mbps，**三个人一起卡**。宁可拦住一个，不能让正在看的两位跟着崩。

- **按 IP 计人头**，不按流数：同一个人 seek / 重连会开好几条流，按流数算会把他当成好几个人。
- **只挡新面孔**：已经在看的人重连一律放行，绝不把他自己挤出去（实测验证）。
- **名额要能单独查**：`<video>` 收到 503 只会触发一个没有响应体的 error 事件，分不清
  「名额满」和「真播不了」，所以加了无副作用的 `GET /api/xifan/slots`，播放页在真去播
  之前先问一句。
- 预转好的走 HLS（读本地磁盘）**不占名额**，只有真要走代理的才检查。

排队提示沿用播放页既有的描稿隐喻；**只有确实存在其他线路时才提示换线路**，
否则只说等位——没有别的线路还让用户去换，那是把提示写成了 bug。

## 2026-08-21 fix(web): 线路二被误当慢源走代理，直接播不了

**效果**：

1. 线路二（`play.xfvod.pro`）恢复可播。此前从「稀饭线路一改走服务端并发分片代理」
   那次提交起就坏了：播放页把**所有** mp4 线路都指向 `/api/xifan/stream`，而代理白名单
   只有 `apn.moedot.net`，线路二直接 502 `不允许代理该地址`。

**修法**：只有**确实需要加速的慢源**才走代理，其余 mp4 保持浏览器裸直连。白名单从
`stream.ts` 导出、注入播放页，播放页按 hostname 判定。

**为什么这不只是修 bug，而是正确的资源策略**：

```text
线路一 apn.moedot.net → 联通网盘   单路 1.4Mbps，非并发不可  → 必须走代理
线路二 play.xfvod.pro              直连 30Mbps，给全套 CORS  → 直连，别碰服务器
```

服务器出口只有 6Mbps（约 2.2 人份），是最稀缺的资源。让线路二走代理不但更慢，
还会把服务器名额白白占掉——**能直连的一律直连，服务器资源只留给只能靠它的线路一**。
顺带：直连线路不再触发预转状态查询，也不占预转配额。

**教训**：改了「所有 mp4 的播放路径」，却只测了线路一。用户日常最常用的恰恰是线路二。

## 2026-08-21 feat(web): 预转配额、LRU 回收与让位观众

**效果**：

1. 预转产物有了硬配额（默认 15G，约 34 集），超了按「最后被看的时间」从旧到新回收。
2. **预转不再影响正在观看的人**：有人在看就冻结预转让出带宽，人走了自动恢复。

**一、配额与回收**

LRU 的排序键是**最后一次被读到的时间**（分片被请求时 `touch` 完成标记的 mtime），
不是「什么时候转的」——转得早不代表没人看。

```text
超配额 → 候选 = 全部已转好的
        ├─ 跳过正在转的
        ├─ 跳过 30 分钟内被读过的   ← 正在看的人，删了就是播放中断
        └─ 剩下的按最后访问时间从旧到新删，删够为止
腾不够 → 拒绝新任务（绝不去动正在被看的那份）
```

实测：4 份各 100MB、最后访问 600/300/120/5 分钟前，配额 450MB，请求腾 150MB →
回收最老的两份；再请求 300MB → 回收第三份；5 分钟前刚看过的那份**始终没被碰**。

**二、让位给观众**（回答「预转会不会影响观看」）

入口带宽只有一份（12 路合计约 5Mbps），观看只要 2.67Mbps，两者同时跑必然互相拖慢。
所以检测到有人在真看就给 ffmpeg 发 `SIGSTOP` 冻住，人走了 `SIGCONT` 解冻。

用 SIGSTOP 而不是杀掉重来：杀掉则 playlist 要从头生成，而暂停是零成本的——它的 TCP
连接闲置后由 stream.ts 的 `CHUNK_SILENCE_MS` 看门狗兜底，恢复时自己重连。

**「谁算观众」必须按读取端标记，不能按 URL 排除**：用户很可能正在看的就是正在预转的
那一集（边看直连版边后台转），按 URL 排除会把真观众一起漏掉，让位直接失效——这是实测
撞出来的。改用只存在于本进程内存的一次性令牌标记预转自己那条流，外部伪造不了。

实测：观众接入 → 立刻「暂停预转让出带宽」，观众 25 秒收到 117MB（4.7MB/s，未被拖慢）
→ 观众离开 → 「观众已散，恢复预转」。

**三、顺手修掉的坑**

ffmpeg 的输入地址原先写死 `127.0.0.1`。生产 node 绑的就是 IPv4 所以能跑，但 vite dev
只监听 `[::1]`，本地一跑就 Connection refused。留了 `XIFAN_INTERNAL_ORIGIN` 给 dev 覆盖。

## 2026-08-21 feat(web): 线路一预转 HLS 分片（最小可用版）

**效果**：

1. 播放页 mp4 线路下方多一条状态栏，登录后可让服务器把这一集预转成 HLS 分片；
   转好后本集自动改用分片版播放（正在播的话就地切换并接着原进度）。

**为什么要转 HLS —— 三段带宽账（全部实测）**：

```text
入口  源站 → 服务器    12 路并发 5.3Mbps                    ✓
出口  服务器 → 手机    单连接 2.2Mbps < 2.67Mbps 码率        ✗
                       同一条链路 8 连接能到 5.22Mbps        ✓
```

`<video>` 直连服务器只开**一条**连接，卡的正是出口那一段。HLS 把整集切片，播放器会
并发拉多片，自然用上多连接。**服务器出口能力本身是好的**（speedtest 上下行均 5.66Mbps），
瓶颈是跨境单连接限速——和线路一源站的现象同源，只是换了个位置发生。

**为什么必须预转，不能边下边切**（这两条都是实测撞出来的）：

1. 边切只有 **1.3x 实时余量**（remux 60s 内容耗时 45s，受入口速度限制），入口一抖就追不上；
2. 边切生成的是 live playlist，用户**跳不到还没切出来的位置**，连跳 OP 都做不了。

**分片用 fMP4 不用 TS**：TS 的 188 字节包头让体积比源涨 12%，fMP4 只涨 8.6%
（实测 TS 3.00Mbps / fMP4 2.90Mbps）。出口只有 5.22Mbps，这 3 个百分点值得省。

**ffmpeg 开销实测**：`-c copy` 只重封装，**CPU 0~1%、峰值内存 60MB**，2 核机器毫无压力。
唯一的成本是磁盘：一集约 490MB。

**几个刻意的设计**：

1. **ffmpeg 的输入指回自己的 `/api/xifan/stream`**，不是源站直连——源站单路只有 1.4Mbps，
   直连会把 remux 饿死。
2. **完成标记 `done` 最后写**：中途崩了目录里虽有分片但没有标记，下次当没转过重来，
   而不是把缺尾巴的 playlist 当成品端出去。
3. **`/prepare` 要登录**：它会写几百 MB、长时间占满入口带宽，不设防就是一个
   「任何人都能让你的服务器免费下片」的按钮。状态查询无副作用，不设限。
4. **串行只跑一个**：remux 不吃 CPU 但入口带宽只有一份，同时转两集会互相拖慢，
   还会跟正在观看的人抢。
5. 分片文件名白名单 + key 必须是 32 位 hex，挡路径穿越。

**还没做（最小版的边界）**：

1. 手动触发，没有「加入追番自动预转」。
2. 没有磁盘配额和 LRU 清理，只有「剩余空间 < 3G 就拒绝新任务」这一道闸；
   `/api/xifan/prepared/list` 和 `DELETE /api/xifan/prepared/:key` 可手动查和删。
3. 没做多集队列。

## 2026-08-21 fix(web): 流代理会话在无人观看时停止下载

**效果**：

1. 连续 seek 不再一次比一次慢。服务器实测三次连续冷 seek，修前 4.03 → 0.17 → 2.35Mbps
   逐次崩塌，修后不再单调递减。

**根因**：读取端断开后，会话要等 `IDLE_MS`(60s) 才收摊，**这 60 秒里 worker 一直在下**。
用户每 seek 一次就多一个后台会话继续下满窗口，反过来抢新会话的带宽：

```text
seek 1 → 会话A 开跑
seek 2 → 会话A 无人读但仍在下 ── 抢 ──> 会话B
seek 3 → A、B 都在下          ── 抢 ──> 会话C   ← 越 seek 越慢
```

**修法**：`claim()` 里发现没有活跃读取端就停手，只留着已下好的窗口等人回来，收摊仍交给
idle 看门狗。**不能直接杀会话** —— `<video>` 暂停时也会断流，一断就杀会把攒好的窗口全丢，
用户一恢复播放就得从头缓冲。
...
## 2026-08-20 perf(web): 稀饭线路一改走服务端并发分片代理

**效果**：

1. 网页版稀饭线路一不再「播几秒卡一下」最后退成官方 iframe。实测经代理后浏览器侧
   拿到 **772KB/s（6.3Mbps）**，是裸直连 221KB/s 的约 4 倍，而该集码率约 2.6Mbps。
2. 起播后 `readyState=4`、时长正确、暂停态即攒到 16.9 秒缓冲，缓冲闸门不再触发回退。
3. 移除了「canplay 后低速静音预缓冲」——实测**有害**（见下）。

**根因**：源站按连接限速。单路 ~170KB/s（1.4Mbps）追不上 2.6Mbps 的码率，6 路并发
合计 ~912KB/s（7.3Mbps）才够。桌面端早有 `mtmedia://`（media-cache.ts）做这件事，
网页版此前是 `<video>` 裸直连，所以只有网页版卡。

**为什么只能放服务端**：最终签名地址 pan.wo.cn **一见 `Origin` 头就 403**（不带是 206）。

```text
浏览器 fetch + MSE 自己开多路   → 必带 Origin → 403         ✗
<video> 裸直连                  → no-CORS 不带 Origin，能播
                                  但 JS 读不到字节，开不了多路 ✗
...
```text
<video src="/api/xifan/stream?u=...">
  ├─ 无 Range / bytes=0-  → 会话：6 个独立 undici Agent(connections:1)
  │                          ├─ 每块各自重走 302 拿签名链（签名链绑连接，不可复用）
  │                          ├─ 字节游标记账：contiguousEnd / nextOffset / completed
  │                          └─ 只推进连续前缀 → ReadableStream → <video>
  └─ 尾部 / 小片段        → 单路直连透传，不碰会话
```

**踩到的四个坑（都会表现成「播放器一个字节都拿不到」）**：

1. **moov 尾部探测顶掉会话**。Chromium 开 mp4 先探文件尾找 moov，那条 `bytes=N-` 会
   被当成冷 seek 把整个会话重开，前面攒的窗口全丢。→ 尾部 / 带结束位的小段走
   `passthrough` 直连，完全不碰会话（对齐桌面端 `TAIL_DIRECT_BYTES`）。
2. **多个读取端互抢内存块**。Chromium 开 mp4 会连开 4 条请求再 abort 掉 3 条，块一发出
   就释放，旧读取端不停就会和新的抢同一批块，两边都攒不出连续前缀。→ 会话加 `reader`
   序号，新读取端接管时旧的安静退场。
3. **首个请求不带 Range 却回了 206**。206 只能回应 Range 请求，媒体栈会把响应体整个
   丢掉——现象是响应头到了但 `encodedBodySize` 恒为 0。→ 无 Range 就回 200。
4. **起播闸门和播放器缓冲闸门互相顶死**。照搬了桌面端 `COLD_START_BYTES`(2MB) 却漏了
   `exposeFirstProgressively`：开场建连要十几秒才攒满 2MB，而播放页的缓冲闸门只等
   10 秒就回退 iframe。→ `regionStart < CHUNK_BYTES`（文件开场）豁免起播闸门。

**移除静默预缓冲的实测依据**：0.0625 倍速会让浏览器判定「缓冲远超消费速度」而主动
限流，10.9 秒只攒到 2.7 秒缓冲；同一时刻 fetch 直接读同一个代理地址能跑 772KB/s。
起播慢的真正原因是带宽不是时机，不需要抢跑。

**多观众共享（同一集只回源一次）**：

会话按「视频 URL + 起始位置」组织，一个会话挂多个读取端：

```text
A 从头看        → 开区间[0-]，6 路下载
B 也从头看      → 命中 A 的窗口，**零回源**，直接吃内存（实测 1.44MB/s）
B 跳到 200MB    → A 没覆盖 → 另开区间[200MB-]，**不顶掉 A**（实测 A 全程不断流）
```

- 下载前沿跟**最远**的读取端走：落在后面的人数据已在内存，不会卡；只有最前面的才可能追上下载端。
- 回收只丢「所有读取端都越过、且已在 KEEP_BEHIND(16MB) 之外」的块；单会话内存超 64MB
  时**请出最慢的读取端**（它重连另开区间），而不是丢块——丢掉某个读取端正要读的块，
  正好就是「看着看着突然卡」。
- 起播闸门加了 6 秒时间上限：多区间并存会抢同一个限速源，实测后来者 14 秒都攒不够
  2MB，只能干等；攒不够也先放行，让播放器先动起来。

**并发路数：12 而不是 6**。实测叠加曲线（每路 12s，全程无一非 206 响应）：

| 并发 | 本机 | 香港 VPS |
|---|---|---|
| 6 路 | 5.25 Mbps | 3.53 Mbps |
| 12 路 | **18.8 Mbps** | **4.93 Mbps** |
| 24 路 | 34.7 Mbps | 4.59（已饱和，再加只是摊薄） |

VPS 上 12 路是拐点，30s 持续复测稳定 5.0~5.3Mbps。**取 12 是有意为之**：该集平均码率
2.67Mbps（474432984B / 1422.1s，VBR 峰值更高，实践上要 3.5~4Mbps 才稳），12 路已是
码率的 7 倍，再往上对播放没有意义，只是多占源站连接、徒增被风控的面。

**光加并发数没用**：`FRONTIER_BAND_BYTES` 必须同步放大。告急窗口原本 4MiB，而
12 路 × 512KiB = 6MiB，一半 worker 会在窗口边缘干等 —— 只提并发不动窗口，实测吞吐
反而从 5.25 掉到 **1.9Mbps**；窗口改成 `2 × WORKERS × CHUNK_BYTES` 后回到 8.2Mbps。

**代理这层有队头阻塞，约折损一半吞吐**：裸并发只统计总字节，12 条谁快谁慢无所谓；
经代理则必须**按序**喂给 `<video>`，第 1 块慢就压住后面 11 块（下好了也只能占着窗口
干等，窗口填满 worker 就停手）。本机 18.8 → 8.2Mbps 即此。这个损耗在「按序输出」
前提下无法消除，只能靠更大窗口缓解。

**还存在的问题**：

1. **服务器端到端未测**。若同比例折损，5.3 × 44% ≈ 2.3Mbps，**低于 3.5~4Mbps 的稳播线**
   ——入口够、但代理这层可能把余量吃掉。上线前必须在 VPS 上实测。
2. **出口 6Mbps 是硬顶**：单人够，2 人贴着上限，3 人必然一起卡。缓存省的是入口，
   省不了出口。
3. **内存窗口不落盘**：后来者只能复用「进度相近」的窗口（KEEP_BEHIND 16MB ≈ 1 分钟）。
4. **两个不同区间同看一集，带宽会被稀释**（实测合计 680KB/s，各约 300~380KB/s）。
5. 若实测不够，走**预下载**路线：往磁盘写不需要按序，没有队头阻塞，12 路能跑满 5.3Mbps，
   番剧每周更新一集、有整整一周时间去下那一小时。
