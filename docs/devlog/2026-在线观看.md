...
## 2026-09-11 fix/feat(web): 快源改走 443 端口；预转分区化——跳哪转哪、半分钟起播、两档码率；手机直连饿死自动改走服务端

**效果**：

1. 手机上直连 `play.xfvod.pro` 播到缓冲耗光、十秒没进展，不再只弹「换线路 / 去源站」，而是保留进度自动切到服务端预转的 HLS 接着看
2. 该域名默认仍直连、不占服务器出口；只有本机实测直连饿死这一次会话才改走，同一地址只救一次
3. `apn.moedot.net` 的慢源链路（预转、名额、心跳）原样复用，没有第二套代理
4. 预转从单档改为 hd（源画质）+ sd（1080p ≤1.2Mbps）两档，播放器自动换档；正在边转边看的人不再把预转冻住
5. **起稿从「攒 15 片」改为「目标位置前方攒够 30 秒」**：源关键帧 10 秒一个时旧规则要等 150 秒内容（≈2 分钟），现在约 25 秒
6. **可以跳到还没转出的位置**：进度条从第一秒起就是整集长度；跳到 7:21 就从 7:18 起转，攒够 30 秒放行，剩下的空洞回头补；手机救援也直接从用户当前进度起转
7. 缓冲闸门先等 1.2 秒再介入（桌面同触屏）：服务器日志抓到桌面网页版「正常播 2 秒 → playback rewound 591.3 → 582.7」——是闸门自己把进度拽回锚点，源站播放器只是顿一下。短抖动现在完全不介入
8. 播放页多两种带胶片的诊断：`seek A -> B (back, in buffer) settled by … in Nms`（缓冲内回退超 1 秒或中途 waiting 才上报）和 `stall recovered after Nms`；暂停时按 -10s 不再误报 `playback rewound`

**真正的根因：我们播的是 `:8088`，源站播的是 443**。在内置浏览器里打开官方播放器（`player.moedot.net/player/index.php?code=xfdm1&from=cf&url=…:8088/…`）
抓它的 `<video>.currentSrc`：`https://play.xfvod.pro/temp/2607/BG02.mp4`——**端口被去掉了**。同一个 Cloudflare 站点、同样的内容和 Range 支持，
但非标端口 8088 在手机运营商网络上走得很差（下面胶片里 64KB 探测 4.8 秒）。`resolve.ts` 现在和官方一样，这个域名一律去端口。
上面 1~3 的救援路径保留作兜底，正常情况下不该再触发。

**Sentry 胶片（iPhone Chrome，直连 play.xfvod.pro:8088）**：

```text
播放中   ahead 16.9 → 0   每 2s 恰好掉 2s ——浏览器一个字节都没在拉
耗光后   ahead 0 → 8.1     26s 才攒 8s（约 0.3x），随后 10s 不动 → gate 判 stall
另一集   ahead 0.1 持续 30s+ 零增长；64KB 探测 4784ms；另有一次 57s 不可达
```

「只有一个人在看」是对的——但直连根本不经过服务器，慢的是这台手机到源站非标端口那一段。
播放页本来就有慢源那条经过验证的链路（VPS 12 路并发 → fMP4 HLS → 手机多片并发拉），缺的只是把它接到直连饿死的判据上。

**关键流程**：

```text
fallbackBufferGate(触屏) → canRescue(url)?
  ├─ 是 → rescued[url]=true → 记 resumeTime → playLine(pl)
  │        └─ needsProxy(url) 现在为 true → 预转轮询 /prepared?rescue=1 → 名额 → 起稿 → HLS 接着播
  └─ 否（已救过 / 域名不在 RESCUE_HOSTS）→ 原来的「换线路 / 去源站」提示
服务端：ALLOWED_HOSTS = PROXY_HOSTS ∪ RESCUE_HOSTS；/prepared 只在 rescue=1 且域名可救时开工
```

**预转改出两档（ABR）**：同一条 ffmpeg、同一份输入，hd 原样 `-c copy`，sd 为 **1080p H.264 crf23 封顶 1.2Mbps**。
主 playlist 按 BANDWIDTH 升序（sd 在前），hls.js / iOS 原生按缓冲水位自动换档。

VPS 实测（2 核 2G，HEVC 1080p 1.85Mbps 源，60s 样本，全部 `nice 19`）：

| 档位 | 速度 | 平均码率 | SSIM vs 源 |
|---|---|---|---|
| 1080p H.264 crf23 ≤1.2M（选用，2 线程） | 1.42× | ≈0.8Mbps | **0.982** |
| 720p crf21 ≤1.6M | 1.90× | ≈0.55Mbps | 0.962 |
| 720p crf23 ≤1.4M | 1.87× | ≈0.42Mbps | 0.958 |
| 1080p H.264 1.8M 定码率 | 1.36× | ≈1.77Mbps（没省） | 0.988 |

番剧画面平坦，**降码率比降分辨率划算**：1080p 降到 0.8Mbps 仍比任何 720p 档清楚，体积差不多。
转码期间 `/api/health` p50 2ms、最差 8ms（1 线程时 33ms），网站不受影响；快源直连本来就不经过服务器。

**分区预转（跳哪转哪）**：

```text
用户跳到 T（还没转出）
  → 播放页 /prepared?from=T → 杀掉正在跑的段（已产出的分片全留）→ 从网格点 S=⌊T/6⌋·6 起新开一段
  → S 前方攒够 30 秒 → 播放页 hls.stopLoad/startLoad + 回到 T
  → 这段跑到尾 / 跑到下一段起点 → 回头补空洞（优先用户当前位置之后的）→ 全补齐才写完成标记
每档的 playlist 由 prepare.ts 按磁盘上的段实时拼：段间 DISCONTINUITY，空洞和片尾用 EXT-X-GAP 撑到整集时长
```

段与段要严丝合缝，靠三件事：sd 从网格点精确起（解码后丢掉 S 之前的帧）且关键帧只钉在 6 秒网格上（`-sc_threshold 0`）；
hd 是 copy，只能从 S 之前最近的源关键帧 K 起——K 从 mp4 的 moov（`moov.ts`：stts/ctts/stss/elst，只读索引不下载画面）算出，
`-hls_time 1` 让 hd 在每个关键帧都切片，任何 K 都是片边界；有界段用 `-t`（按输入时间收口，两档同在 E 停）。
曾试 `-copyts` 让 tfdt 带绝对时间——hls 复用器把 tfdt 归零、偏移塞进 elst，ffprobe 读出两倍的 pts，弃用。
`npm run test:prepare-regions`（本机合成 4 分钟视频 + 限速假源站）：0 起转 → 跳 150s 杀段重开 → 补 18→150 → 两档总长 240.00、无 GAP、段起点 sd 0/18/150、hd 0/10/150，14 项通过。

**昨天那次「起稿两分钟、2:30 卡死」的根因**：起稿慢是「15 片」× 源关键帧 10 秒 = 150 秒内容；2:30 卡死是下面这个死结——

**顺手修的一个死结**：08-29 接入公共名额后，预转让位改看 `activeAdmissionCount`——它把**正在边转边看这一集**的
HLS 观众也算成「占入口的人」，ffmpeg 被 SIGSTOP 冻住，播放器 90 秒后追上转码前沿必卡。改回只数经代理直连拉流的
外部读取端（`externalViewerCount`），HLS 观众读的是磁盘分片，不占入口。

**边界**：moov 读不到（极少数非常规封装）就退回旧的顺序转、不能跳；跳进空洞时 hls.js 报非致命 `fragGap`、页面盖浮层等
那一段就绪再回到目标点，Safari 原生 HLS 按 EVENT 周期自己刷新 playlist（最多慢一个分片长度）；源关键帧间隔 < 1 秒的
文件 hd 片边界不是每个关键帧都有，可能留下 ≤1 片的缝（真实源 10 秒一个，未见）。一集约多占 200MB，配额 15G 从 34 集降到约 23 集；sd 档 1.42× 只比 remux 的 1.3× 略快，起稿时间不变（约 1 分钟）；
旧格式（单档 `seg%05d`）目录继续可播、不重转。桌面端不走救援（有 1/16 倍速保活，且退 iframe 路径未见事故）；
判定后是否真的顺畅要看发布后的 Sentry 胶片，`rescue:` 这行日志就是回溯点。

## 2026-08-31 fix(web): 修复稀饭播放页代理恢复与手动起播

**效果**：

1. 本地 Clash/TUN 比开发服务晚启动、或启动探测错过代理时，第一次真实传输失败会重新对齐已存在的本地代理
2. 拿到媒体地址后只设置 `src` 并保持暂停；`canplay` 收起「描线中」纸片，用户点 `<video controls>` 的浏览器原生播放控件后直接进入 `playing`，首次起播不再误触发缓冲纸片或自动播放失败提示
3. 手机、平板和桌面端共用同一条手动起播路径；断网恢复只在此前确实正在播放时恢复，不把首次打开当成自动播放
4. 媒体错误、进度倒退等真正的播放诊断仍由页面监控直接上报，出站事件保留经清洗的 User-Agent 供 Sentry 解析浏览器 / 操作系统 / 设备；服务端日志仍保留作监控脚本未加载时的兜底，带页面 SDK 标记的事件不重复开 issue

![稀饭播放页代理恢复与手动起播](../devlog-assets/xifan-web-startup-recovery.svg)

**本地代理冷启动根因（由日志与代码路径共同指向）**：

```text
开发服务启动 → 直连稀饭失败 → 当时 Clash 控制器 / 混合端口尚未就绪
  → 一次性代理探测结束且没有可用代理
  → Clash 随后恢复，但 proxyReady 不会再次执行
  → /playlist 继续直连 → fetch failed，线路区还没画出来就结束
```

**首播纸片闪回根因（由播放器事件链确认）**：

```text
首次拿到 MP4 → 旧逻辑预先 gateOnPlay=true
  → canplay 收起初始纸片、控件出现
  → 用户点击原生播放控件 → playing
  → playing 误被当成“中途卡顿” → beginBufferGate()
  → 又显示「描线中」→ 缓冲目标达到后才收起
```

这不是自动播放失败，也不是 iframe 回退；是把“首次起播”和“播放中途缓冲”共用了同一个闸门入口。
现在首次 `playing` 只标记 `playbackStarted` 并保持纸片隐藏，只有后续真实 `waiting` / `seek` 才允许开启缓冲闸门。
这条防回归规则已补入 [`AI_GUIDELINES.md`](../../AI_GUIDELINES.md) 和 [`网页版播放 ideas`](../ideas/012-网页版.md)，以后改播放事件先按固定序列复测。

**首次播放状态流**：

```text
拿到线路 → 设置 src / load → canplay → 收起手帐纸片，保持 paused
  → 用户点 `<video controls>` 原生播放控件 → playing → 直接正常播放
  → 后续真实 waiting / seek → 才进入缓冲闸门
```

这里的“原生播放键”只指主页面 `<video controls>` 由浏览器绘制的播放控件：桌面通常是底部控制条左侧按钮，移动端位置可能由浏览器调整；它不是画面中央的自定义纸片按钮。若直连失败退到官方 `iframe`，父页面不模拟点击，改由 iframe 内稀饭播放器自己的播放控件接管（位置由对方播放器决定）。

重新对齐只由真实传输层失败触发；并发请求共享一次本地端口检查，5 秒内不重复，原有那一次瞬时重试仍是唯一的源站请求重试。

**监控分工**：

- 每条诊断仍进页面面包屑和服务端 stdout；媒体错误、进度倒退等失败诊断由页面 SDK `captureMessage` 直接发到浏览器项目
- 页面 SDK 上报后在回传日志带 `sdk: 1`，服务端只打印不再重复转发；SDK 缺失时才由服务端按小时配额转发，User-Agent 仅作为额外数据，不冒充设备上下文
- 继续保持 `sendDefaultPii: false`、URL 去 query、无 Session Replay；设备上下文验证放到发布后的受控设备测试，不用第三方源站作测试靶点

**边界 / 待发布验收**：

1. 页面 SDK 没加载、被拦截或旧浏览器不支持时，Sentry 只能收到服务端 Node / Ubuntu 上下文；这条路径仍保留
2. 首次打开不自动播放，不刷新签名、不换线路、不增加媒体请求；主页面原生播放控件只复用当前媒体地址，iframe 回退则由对方播放器控件接管，移动端仍由触屏判据走原有播放路径
3. 代理环境变量仍拥有最高优先级；生产 VPS / Vercel 不启用本地自动对齐。若本机 Clash 没有可读混合端口，仍需显式设置 `HTTPS_PROXY`

## 2026-08-29 feat(web): 稀饭播放器接入公共慢源候补

**效果**：

1. 稀饭慢源不再按 IP 碰运气重试：满员后自动进入账号级候补，有月卡或放映券时进入优先队列，轮到后保留五分钟
2. 候补属于公共容量池而不是某部番；等待和观看期间可换番、换集、换慢源而不重排、不重复扣券，快源仍直接播放
3. 页面关闭后候补继续保留；本站仍开着时显示站内提示，完全离线时向已验证邮箱通知。连续暂停 10 分钟或失联 3 分钟会释放名额
...
播放器请求慢源
  → 公共容量池按账号分配候补 / 五分钟保留
  → MP4 或 HLS 首个真实媒体请求认领名额并扣除已锁定放映券
  → 播放心跳区分正常播放 / 暂停 / 页面离线
  → 超时释放时截断该账号仍挂着的代理长连接，再放行下一位
```

**关键边界**：

1. 一个账号在同一容量池只有一个播放器所有权；新标签页接管时旧播放器不能继续并发拉流，HLS 分片与 MP4 代理都校验当前播放器会话
2. 优先与普通候补按“连续 2 位优先、再 1 位普通”放行；同级用单调入队顺序保证同毫秒并发时仍严格 FIFO
3. 稀饭与 Girigiri 的站内播放入口、线路解析和源站跳转现在都要求 MapleTools 登录；当前 Girigiri 尚无慢源，只预留复用公共容量池的边界
...
**关键点二 · 代理自动对齐**（`server/http.ts` 的启动对齐 + `proxyReady`）：
...
启动时每个候选真发一次 HEAD 验证，通过才切换传输层；真实请求若遇传输失败，只检查本地端口后沿用原有单次重试，避免把冷启动错过代理焊死在直连。
```
...
`await proxyReady`，保证首个请求不会抢在启动探测定盘前走错传输层
