...
2. 自动并入现有邮箱账号的前提是 `email_verified === true`：Google 核验的邮箱与本站「验证码证明邮箱控制权」证明同一事实，等价于完成一次验证码登录；**Google 未核验的邮箱一律不参与匹配、也不写进账号**（视为没有邮箱，建一个仅靠 Google 身份登录的无密码账号）——杜绝拿未核验邮箱冒名顶替别人的邮箱账号
