...
- WBI 签名是这次唯一新引入的机制（登录、播放地址走的是另一套 TV 端 appkey 签名，互不相关），参照 `/Users/example-user/Downloads/biu` 项目 `electron/ipc/api/wbi.ts` 的同一套算法：`/x/web-interface/nav` 取 `img_key`/`sub_key` → 按固定表打乱拼接成 32 位 `mixin_key` → 参数排序拼接 + `wts` 时间戳 → `md5(query + mixin_key)`。密钥按天轮换，进程内缓存一份，真被拒了（`-403`/`-352`）才强刷一次重试，不是每次搜索都打一遍 `nav`。用真实接口跑过一遍验证算法正确（`code:0`，拿到了带 `<em class="keyword">` 高亮标签的真实标题）
