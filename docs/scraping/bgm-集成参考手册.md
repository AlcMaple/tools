...
- [`bgm-简介解析-回归用例.md`](../regression/bgm-简介解析-回归用例.md) —— 简介中日混排解析的回归用例
- [`ideas/archive/003-BGM限流防御与错误反馈.md`](../ideas/archive/003-BGM限流防御与错误反馈.md) —— 限流防御一期归档记录
- [`ideas/archive/004-BGM登录与认证.md`](../ideas/archive/004-BGM登录与认证.md) —— **登录方案否决记录（重点看 §2.7）**
...
**与别名回退的关系**（不能破坏）：别名搜索（`visibleMatch` 全程为空）也吃这条规则、会在第 2 页就早停 —— 这**正好**，因为别名回退靠「连续 miss 早停」沿命中带扫、第 1 页通常就够。早停前用 gate 卡住：`visibleCount > 0 || unmatchedCount >= ALIAS_LOOKUP_MIN_CANDIDATES`(=4)，保证首页结果太少（<4 条）时不会过早停掉、导致别名回退没东西可查。

**近命中保留**（用户拍板，**全局生效，不分路径**）：用户常按「俗称 / 大众名」搜（如「侦探已死」/ 系列名「黑之契约者」），想要的续作 / 同系列（「侦探已经死了。第二季」/「DARKER THAN BLACK –流星的双子–」）标题和别名都可能不含这个词、本会被过滤掉 —— 但它紧挨命中条目、通常正是用户想要的。规则：`searchBgm` 单遍按 BGM 相关度顺序处理 `allItems`，被「连续 N 个 miss」**早停**收尾时，把**触发那一页**上「已检查过的 miss」也一并当结果返回（含 3-miss streak 本身 + 同页更早的零星 miss）。**只限触发页**：上一页 / 更早页的 miss 离命中带更远、不回收（每条 item 在 `parsePage` 后标了 `page` 字段供区分）。锚点 `sawHit`：本次确实命中过（可见命中或别名命中）才回收，避免「整页零命中」的搜索返回一整页纯噪声。

> **关键：早期版本把这段逻辑错误地嵌在 `matched.length === 0`（零可见命中）分支里**，导致「黑之契约者」这种标题直接命中的搜索走了「只返回可见命中」的快路径、整段保留逻辑被跳过 → 漏掉「流星的双子」。已改为单遍全局处理（不再有 `matched`/`unmatched` 两段式拆分）。**别再把它退回到只在零可见命中时生效**。

**近似匹配层（2026-07-27 加）**：`visibleMatch` 和别名判定在「连续子串」之外再补一层归一化编辑距离 —— 相似度 `1 - 编辑距离 / 较长串长度 ≥ 0.7`，且两串都 ≥ 3 字、长度差 ≤ 2。起因是真实漏检：搜「永久的黄昏」在 bgm.tv 第一条就是《永远的黄昏 / 永久のユウグレ》，我们却回「未找到」—— 主标题、日文副标题、四个别名**没有任何一个**包含「永久的黄昏」这个连续子串（用户把日文名的「永久」和中文名的「的黄昏」拼在了一起），子串判定必然落空。BGM 服务端是分词 / 字符级宽匹配，认得出来；我们只认子串，比它严得多。阈值对 4~7 字标题实际等价于「最多差 1 个字」，长标题随长度自然放宽。**近似必须逐字段算**（主标题、副标题、每个别名各算一次），拿拼接串算会把长度差 gate 直接顶掉。

**零命中兜底**：整轮过滤后 `matched` 仍为空时，返回 BGM 相关度最前的 `ZERO_MATCH_FALLBACK_LIMIT`(=3) 条。判据：真没有这个番时 BGM 自己返回空结果页、`page1Items.length === 0` 已提前 `return []`；能走到这里说明 BGM 明明回了结果、是**我们的过滤判错了**，此时返回空 = 用户看到「未找到」而 bgm.tv 上明明有，是最坏的结果。与「近命中保留」的 `sawHit` 锚点不冲突 —— 那条限制的是「回多少」，这里只回 3 条，不会把整页噪声倒给用户。

**miss 的定义 = 标题和别名都没命中**（用户原话）。所以单遍处理时：标题命中 → 直接收（零 API）；标题没命中 → **验别名**，别名命中也收、否则才算 miss。验别名只发生在「标题没命中」的条目上，且连撞 N 个 miss 即停 —— 标题直接命中的项零 API，命中带后面通常 N 个 miss 就早停。这样既能可靠捞到「靠别名命中的同系列」（不只是位置碰巧落在 streak 里的），也不会无脑全验。失控扫描由 L2 滚动窗口 + L3 熔断 + 分页早停（候选池天然 ~2 页）兜底。

代价（已知并接受）：触发页上夹在命中带后的无关条目（如搜「黑之契约者」带进的小花仙 / 潘多拉之心）也会被一起返回 —— 用户明确选择「宁可多点噪声也别漏掉想要的」。
...
- **L1 缩小单次突发**：别名回退的**唯一 per-search 限制器**是「连续 miss 早停」（`ALIAS_LOOKUP_MAX_CONSECUTIVE_MISSES`=3，命中重置）。用户基本每次搜索都走中文别名回退、单次几个 API，是限流主因之一。**注意不要加任何「候选池硬帽」**（`slice(0,N)` 或「最多查 N 条」）：那会让头部一个热门噪声条目占掉名额、把真命中挤出窗口（搜「谭雅战记」漏掉「幼女战记 第二季」就是这么来的），且纯别名的大系列会被整段截断 —— 砍候选池只砍召回、不救召回。失控扫描（hit/miss 交错永远触发不了连续-miss 早停）由 L2 滚动窗口 + L3 熔断兜底，分页早停又把纯别名候选池天然限制在 ~2 页，扫描量本就有界。现行做法沿 BGM 相关度顺序逐条验、命中重置 miss 计数、连撞 3 个 miss 收尾，命中带多长扫多长。不动分页早停（结果数不变）。
...
完整决策记录见 [`ideas/archive/004-BGM登录与认证.md`](../ideas/archive/004-BGM登录与认证.md)。摘要：
...
详细回归用例 + 改动前 checklist 见 [`bgm-简介解析-回归用例.md`](../regression/bgm-简介解析-回归用例.md)。
...
- **2026-05-23** —— **§3 新增「分页早停」**。`searchBgm` 翻页旧逻辑硬翻到 `totalPages`，但 BGM 按相关度排序、命中带之后全是噪声（搜「光之美少女」命中带 8 页 / `totalPages` 82，9-82 页纯浪费 ≈ 3 分钟限流红线试探）。改为连续 2 页无 `visibleMatch` 就早停，gate（`visibleCount>0 || unmatchedCount>=ALIAS_LOOKUP_MIN_CANDIDATES`）保证别名回退候选不被过早截断。少发请求 = 最好的限流防御
- **2026-05-31** —— **别名回退去掉 `slice(0,N)` 死板候选帽**（修 008 副作用）。现象：搜「谭雅战记」只回 3 条、漏「幼女战记 第二季」。根因：008 把候选帽从 8 砍到 4，而 BGM 把高人气的「罗小黑战记2」(字符碎片命中「战记") 排在第 1 占掉一个名额，`unmatched.slice(0,4)` 正好把真命中的第 5 个「第二季」切出窗口、从没验过别名。**砍候选池只砍召回、不救召回**。改为：删 `ALIAS_LOOKUP_LIMIT`，沿 BGM 相关度顺序逐条验，**唯一** per-search 限制器=连续 miss 早停（=3，命中重置），**不再加任何候选池硬帽**（先加了个 `HARD_CAP=8` 兜底，但讨论后发现它对纯别名大系列照样截断召回 → 去掉；失控扫描交给 L2 滚动窗口 + L3 熔断 + 分页早停天然把候选池限制在 ~2 页）。该省的 API 全省在噪声、不再误伤命中带。注意：别名回退**只在整页零 visibleMatch 时触发**；像「光之美少女」这种标题可见命中的搜索全走 `matched`、零 API、不受此路径影响
- **2026-05-31** —— **别名回退加「近命中保留」**（同次讨论）。现象：搜俗称「侦探已死」只回主条目、漏「侦探已经死了。第二季」（续作标题和别名都不含俗称 → 被过滤）。规则：被「连续 3-miss」早停收尾时（且确实命中过作锚点），把**触发页**上已检查过的 miss 也当结果保留（只限触发页，上一页 miss 不算；每条 item 标 `page` 字段区分）。代价：触发页夹带的无关条目（柯南剧场版）也会返回 —— 用户拍板「宁可多噪声也别漏」。详见 §3「近命中保留」
- **2026-05-31** —— **「近命中保留」改为全局生效**（同次讨论续）。现象：搜系列名「黑之契约者」只回 2 条标题命中项、漏同系列的「DARKER THAN BLACK –流星的双子–」（标题是"流星的双子"、别名也不含"黑之契约者" → 是真 miss）。根因：上一条把保留逻辑嵌在 `matched.length === 0` 分支里，标题直接命中的搜索走快路径、整段被跳过。改为**单遍全局处理**（删掉 `matched`/`unmatched` 两段式），保留逻辑无差别生效；锚点从 `matched.length>0` 换成 `sawHit`。miss 定义统一为「标题和别名都没命中」：标题命中零 API，标题没命中才验别名（中间一度试过「有可见命中就不验别名」省 API，但那样会漏掉靠别名命中的同系列，已回退成始终验）。验别名仍受连续-N-miss 早停 + L2/L3 约束。详见 §3「近命中保留」
- **2026-07-27** —— **§3 新增「近似匹配层」+「零命中兜底」**。现象：搜「永久的黄昏」返回「未找到」，而 bgm.tv 上《永远的黄昏 / 永久のユウグレ》就排第一。根因：主标题、日文副标题、四个别名（永恒余晖 / 永遠的黃昏 / Towa no Yuugure / Dusk Beyond the End of the World）**没有任何一个**包含「永久的黄昏」这个连续子串——用户把日文名的「永久」和中文名的「的黄昏」拼在了一起；BGM 服务端是分词 / 字符级宽匹配认得出来，我们只认子串比它严得多，于是当噪声过滤掉、三条 miss 连撞早停、`sawHit` 从未成立 → 整轮过滤返回空。修复：① 子串落空后补一层归一化编辑距离（相似度 ≥ 0.7、两串 ≥ 3 字、长度差 ≤ 2，逐字段算），「永久的黄昏」vs「永远的黄昏」距离 1 → 命中；② 整轮过滤后仍为空时回退 BGM 相关度前 3 条（真没有该番时 BGM 返回空结果页、上游已提前 return，走到这里就说明是我们判错了）。回归验证：谭雅战记 / 侦探已死 / 黑之契约者 / 出包王女 四个历史用例结果不变
