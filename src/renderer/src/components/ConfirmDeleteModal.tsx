// 通用的删除确认弹窗。
//
// 项目里轻量场景用的是行内确认(点一下 trash 变红、再点才真删)。但追番这种重对象的删除
// 用户更需要「看到一个弹窗、明确知道自己在干嘛」—— 尤其是自定义标签、最爱值、鉴赏神回这些
// 本地数据会一起没。
//
// Props 少而准:标题 / 描述 / 对象名三段就能覆盖绝大多数删除确认。danger 默认 true(红色按钮)
// 极少数「删但不危险」的场景传 false。
...
interface Props {
  ...
  description: string
  /** 可选:被删对象的名字,高亮显示在描述上方,让用户二次确认目标对不对。 */
  itemName?: string
  /** 确认按钮文案，默认「删除」。 */
  confirmText?: string
