// HLS 分片滑动窗口预取 —— 只服务 media-proxy 的分片分支(Girigiri 走这条)。
//
// 为什么要它(011「Girigiri 在线播放卡顿」的结论,别再被直觉带偏):
// 「HLS 分片」和「NDM 式多线程分段下载」看着都是分片,其实不是一回事。hls.js 的默认
// loader 是**单连接、一个分片接一个分片顺序取**,不会开多条连接去抢同一段时间窗口;
// 而 girigiri/download.ts 的下载路径本来就是 8 路并发抓 .ts —— 下载快、播放卡,是同一份
// 资源两条路径不对称造成的,不是站点只对播放限速。
//
// 做法:代理在重写播放列表时把分片顺序记下来(rememberPlaylist);hls.js 来要第 N 个分片时,
// 除了回它这一片,再并发把后面 PREFETCH_AHEAD 片提前抓进内存。等 hls.js 顺序要到它们时
// 直接从内存回,把单连接顺序取的往返延迟摊掉。
//
// 分片是独立小文件,**不需要按字节区间切**(那是 mp4 直链那条路的事,见 media-cache.ts)。
import { net } from 'electron'

/** 每次命中往前预取多少片。Girigiri 分片多为 5~10s,6 片≈半分钟缓冲。 */
const PREFETCH_AHEAD = 6
/** 与 girigiri/download.ts 的下载并发同口径,别再往上加 —— 同源并发过高是封 IP 的快车道。 */
...
/** 当前播放列表的分片顺序(绝对地址,即代理 URL 里的 u 参数原值)。 */
let order: string[] = []
...
/** 换集/换源/退出播放页时调用,连同 media-cache 一起收摊。 */
...
/**
 * 记住一份播放列表的分片顺序。同一个列表重复解析(hls.js 会周期性重拉直播列表)不清缓存,
...
function fetchSegment(url: string, headers: Record<string, string>): Entry {
  ...
  entry.p = withSlot(async () => {
    try {
      ...
      // 都不该由预取层去加压(见 AI_GUIDELINES「无应用层重试」红线)。
      entry.bytes = buf
