...
import type { GirigiriSearchResult, GirigiriWatchInfo } from '../types/girigiri'
...
type PageState =
  | { status: 'idle' }
  | { status: 'captcha'; imageB64: string; keyword: string; captchaSource: Source; captchaError?: string }
  | { status: 'verifying'; keyword: string; captchaSource: Source }
  | { status: 'searching' }
  | { status: 'results'; cards: SearchCard[]; keyword: string }
  | { status: 'xifan_config'; cards: SearchCard[]; card: SearchCard; watchInfo: XifanWatchInfo }
  | { status: 'girigiri_config'; cards: SearchCard[]; card: SearchCard; watchInfo: GirigiriWatchInfo }
...
function normalizeXifan(r: XifanSearchResult): SearchCard {
  return { title: r.title, cover: r.cover, year: r.year, tag: r.area, count: r.episode, key: r.watch_url, source: 'Xifan' }
}

function normalizeGirigiri(r: GirigiriSearchResult): SearchCard {
  return { title: r.title, cover: r.cover, year: r.year, tag: r.region, count: '', key: r.play_url, source: 'Girigiri' }
}
...
function XifanDownloadConfigModal({ card, watchInfo, onClose, onStart }: XifanConfigProps): JSX.Element {
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-surface-container-lowest/60 backdrop-blur-sm">
      <div className="bg-surface-container w-full max-w-lg rounded-xl border border-outline-variant/20 p-8">
        <div className="flex items-start justify-between mb-6">
          ...
          <button onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-surface-container-high transition-colors text-on-surface-variant/60">
            ...
            <div className="space-y-2">
              {validSources.map((src) => (
                <label key={src.idx} className={`flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition-colors ${selectedIdx === src.idx ? 'border-primary/40 bg-primary/5' : 'border-outline-variant/20 hover:bg-surface-container-high'}`}>
                  <input type="radio" name="source" value={src.idx} checked={selectedIdx === src.idx} onChange={() => setSelectedIdx(src.idx)} className="accent-primary" />
                  <span className="font-label text-sm text-on-surface">{src.name}</span>
                  <span className="ml-auto font-label text-[10px] text-on-surface-variant/40 uppercase tracking-widest">Source {src.idx}</span>
                </label>
              ))}
            </div>
          </div>
        ...
        <div className="mb-8">
          ...
          <div className="flex items-center gap-4">
            <div className="flex-1">
              ...
              <input type="number" min={1} max={watchInfo.total} value={startStr}
            ...
            <div className="flex-1">
              ...
              <input type="number" min={1} max={watchInfo.total} value={endStr}
        ...
        <div className="flex gap-3">
          <button onClick={onClose} className="flex-1 py-3 rounded-xl border border-outline-variant/20 text-sm font-label text-on-surface-variant hover:bg-surface-container-high transition-colors">Cancel</button>
          <button onClick={handleStart} disabled={validSources.length === 0}
...
interface GirigiriConfigProps {
  ...
  onClose: () => void
  onStart: (selectedIdxs: number[]) => void
}

function GirigiriDownloadConfigModal({ card, watchInfo, onClose, onStart }: GirigiriConfigProps): JSX.Element {
  const eps = watchInfo.episodes
  const [startStr, setStartStr] = useState('1')
  const [endStr, setEndStr] = useState(String(eps.length))

  const clampStart = (s: string): number => Math.max(1, Math.min(eps.length, parseInt(s, 10) || 1))
  const clampEnd = (s: string, start: number): number => Math.max(start, Math.min(eps.length, parseInt(s, 10) || eps.length))

  const handleStart = (): void => {
    const s = clampStart(startStr)
    const e = clampEnd(endStr, s)
    // Select episodes by positional range (1-based), return their idx values
    const selected = eps.slice(s - 1, e).map((ep) => ep.idx)
    onStart(selected)
  }
      ...
      <div className="bg-surface-container w-full max-w-lg rounded-xl border border-outline-variant/20 p-8">
        ...
        {eps.length > 0 && (
          <div className="mb-6 p-3 bg-surface-container-low rounded-lg border border-outline-variant/10 max-h-40 overflow-y-auto">
            <div className="grid grid-cols-3 gap-1.5">
              {eps.map((ep) => (
                <div key={ep.idx} className="px-2 py-1 rounded bg-surface-container-high text-[10px] font-label text-on-surface-variant/60 truncate">
                  {ep.name}
                </div>
              ))}
            </div>
        ...
        <div className="mb-8">
          ...
          <div className="flex items-center gap-4">
            <div className="flex-1">
              <label className="font-label text-[10px] text-on-surface-variant/40 uppercase tracking-widest block mb-1.5">From</label>
              <input type="number" min={1} max={eps.length} value={startStr}
                onChange={(e) => setStartStr(e.target.value)}
                onBlur={() => setStartStr(String(clampStart(startStr)))}
                className="w-full bg-surface-container-highest border border-outline-variant/20 rounded-lg px-3 py-2.5 text-sm font-label text-on-surface outline-none focus:border-primary/40 transition-colors" />
            </div>
            ...
            <div className="flex-1">
              <label className="font-label text-[10px] text-on-surface-variant/40 uppercase tracking-widest block mb-1.5">To</label>
              <input type="number" min={1} max={eps.length} value={endStr}
                onChange={(e) => setEndStr(e.target.value)}
                onBlur={() => setEndStr(String(clampEnd(endStr, clampStart(startStr))))}
                className="w-full bg-surface-container-highest border border-outline-variant/20 rounded-lg px-3 py-2.5 text-sm font-label text-on-surface outline-none focus:border-primary/40 transition-colors" />
            </div>
            <div className="mt-5">
              <span className="font-label text-[10px] text-on-surface-variant/30">/ {eps.length}</span>
            </div>
          </div>
          <p className="font-label text-[10px] text-on-surface-variant/30 mt-3 leading-relaxed">
            Each episode uses Playwright to capture the stream link — download may take longer than Xifan.
          </p>
        </div>

        <div className="flex gap-3">
          <button onClick={onClose} className="flex-1 py-3 rounded-xl border border-outline-variant/20 text-sm font-label text-on-surface-variant hover:bg-surface-container-high transition-colors">Cancel</button>
          <button onClick={handleStart} disabled={eps.length === 0}
            className="flex-1 py-3 rounded-xl primary-gradient text-on-primary text-sm font-black tracking-widest hover:opacity-90 transition-opacity disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2">
            <span className="material-symbols-outlined text-base leading-none">bolt</span>
            START DOWNLOAD
          </button>
...
function SearchDownload(): JSX.Element {
    ...
    try {
      if (source === 'Girigiri') {
        ...
        if (!Array.isArray(result) && result.needs_captcha) {
          ...
          setCaptchaInput('')
          setState({ status: 'captcha', imageB64: image_b64, keyword, captchaSource: 'Girigiri' })
        ...
        if (!Array.isArray(result) && result.needs_captcha) {
          ...
          setCaptchaInput('')
          setState({ status: 'captcha', imageB64: image_b64, keyword, captchaSource: 'Xifan' })
        } else if (Array.isArray(result)) {
      ...
      if (success) {
        ...
        if (captchaSource === 'Girigiri') {
          ...
          if (Array.isArray(result)) {
            ...
            const { image_b64 } = await window.girigiriApi.getCaptcha()
            setCaptchaInput('')
            setState({ status: 'captcha', imageB64: image_b64, keyword, captchaSource, captchaError: 'Verification failed, please retry.' })
          }
        } else {
          const result = await window.xifanApi.search(keyword)
          if (Array.isArray(result)) {
        ...
        const { image_b64 } = await api.getCaptcha()
        setCaptchaInput('')
        setState({ status: 'captcha', imageB64: image_b64, keyword, captchaSource, captchaError: 'Wrong code, try again.' })
      }
    } catch (err) {
      setState({ status: 'error', message: String(err) })
    }
  ...
  const handleDownloadClick = async (card: SearchCard): Promise<void> => {
    ...
    try {
      if (card.source === 'Girigiri') {
        ...
        if (watchInfo.error) { alert(`Failed to load episodes: ${watchInfo.error}`); return }
        setState({ status: 'girigiri_config', cards, card, watchInfo })
      } else {
        if (isSearchCacheEnabled()) {
          const cached = getCachedXifanWatch(card.key)
          if (cached) { setState({ status: 'xifan_config', cards, card, watchInfo: cached }); return }
        }
        const watchInfo = await window.xifanApi.getWatch(card.key)
        if (watchInfo.error) { alert(`Failed to load sources: ${watchInfo.error}`); return }
  ...
  const handleStartXifanDownload = async (templates: string[], startEp: number, endEp: number): Promise<void> => {
    ...
    try {
      ...
      downloadStore.addTask({
        ...
        startEp, endEp, templates, savePath, status: 'running',
        epStatus, epProgress: {}, startedAt: Date.now(), pid,
      })
  ...
  const handleStartGirigiriDownload = async (selectedIdxs: number[]): Promise<void> => {
    if (state.status !== 'girigiri_config') return
    const { card, cards, watchInfo } = state
    const title = watchInfo.title || card.title
    const savePath = getSavePath()
    const selectedEps = watchInfo.episodes.filter((e) => selectedIdxs.includes(e.idx))
    try {
      const { taskId } = await window.girigiriApi.startDownload(title, watchInfo.episodes, selectedIdxs, savePath)
      const epStatus: Record<number, 'pending'> = {}
      ...
      downloadStore.addTask({
        ...
        templates: [], girigiriEps: selectedEps, savePath, status: 'running',
        epStatus, epProgress: {}, startedAt: Date.now(),
      })
      setDownloadStarted(true)
      setTimeout(() => setDownloadStarted(false), 3000)
      setState({ status: 'results', cards, keyword: currentKeyword.current })
  ...
  const captchaSourceLabel = state.status === 'captcha' ? (state.captchaSource === 'Girigiri' ? 'Girigiri Love' : 'Xifan ACG') : ''
    ...
    <div className="min-h-full bg-background">
      ...
      <main className="pt-16 px-8 py-8">
        ...
        <section className="mt-10 mb-12">
          <p className="font-label text-xs text-primary/70 tracking-[0.3em] uppercase mb-3">Multisource Downloader</p>
          ...
          <p className="text-on-surface-variant/60 text-sm max-w-lg font-label">
            Search and retrieve anime from multiple sources simultaneously. Automated pipeline from search to archive.
          </p>
        </section>
        ...
        <section className="mb-10">
          <div className="flex items-center gap-3 max-w-3xl">
            <div className="flex-1 flex items-center bg-surface-container-highest rounded-xl px-5 py-3.5 space-x-3 border border-outline-variant/30 focus-within:border-primary/40 transition-colors">
              <span className="material-symbols-outlined text-primary text-xl leading-none">travel_explore</span>
              <input
                type="text"
                value={searchQuery}
            ...
            <div className="relative">
              ...
              <span className="material-symbols-outlined absolute right-2 top-1/2 -translate-y-1/2 text-on-surface-variant/60 text-sm pointer-events-none leading-none">expand_more</span>
            </div>
        ...
        {state.status === 'idle' && (
          <div className="flex flex-col items-center justify-center py-24 gap-4 text-on-surface-variant/20">
            ...
            <p className="font-label text-xs tracking-widest uppercase">Enter a keyword to begin</p>
          </div>
        )}
        ...
        {state.status === 'error' && (
          <div className="flex flex-col items-center justify-center py-24 gap-4">
            ...
            <button onClick={() => setState({ status: 'idle' })} className="font-label text-xs text-primary hover:underline">Try again</button>
          </div>
        )}
        ...
        {state.status === 'results' && state.cards.length === 0 && (
          <div className="flex flex-col items-center justify-center py-24 gap-4">
            ...
            <p className="font-label text-sm text-on-surface-variant/50">
              No results found for <span className="text-primary">"{state.keyword}"</span>
            </p>
          </div>
        )}

        {(state.status === 'results' || state.status === 'xifan_config' || state.status === 'girigiri_config') && state.cards.length > 0 && (
          <section>
            ...
            <div className="grid grid-cols-4 gap-4">
              {state.cards.map((card, idx) => (
                <div key={idx} className="group relative bg-surface-container rounded-xl overflow-hidden border border-outline-variant/20 hover:border-primary/30 transition-all duration-300">
                  <div className="aspect-[2/3] relative overflow-hidden">
                    {card.cover ? (
                      <img src={card.cover} alt={card.title} className="w-full h-full object-cover" onError={(e) => { e.currentTarget.style.display = 'none' }} />
                    ) : (
                  ...
                  <div className="p-3">
                    ...
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-label text-on-surface-variant/50">
                        {card.year}{card.tag ? ` · ${card.tag}` : ''}
                      </span>
                      {card.count && <span className="text-xs font-label text-primary/70">{card.count}</span>}
                    </div>
                  </div>
                </div>
              ))}
          ...
          <div className="glass-effect w-full max-w-md rounded-xl p-8 border border-outline-variant/30">
            <div className="flex items-start justify-between mb-6">
              <div className="flex items-center space-x-3">
                <div className="w-10 h-10 rounded-xl bg-primary/10 flex items-center justify-center">
                  <span className="material-symbols-outlined text-primary text-xl leading-none">security</span>
                </div>
                <div>
                  <h3 className="text-sm font-black tracking-wider text-on-surface">CRAWLER VERIFICATION</h3>
                  <p className="text-[10px] font-label text-on-surface-variant/50 mt-0.5">Source: {captchaSourceLabel}</p>
                </div>
              </div>
              <button onClick={() => setState({ status: 'idle' })} className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-surface-container transition-colors text-on-surface-variant/60 hover:text-on-surface">
                <span className="material-symbols-outlined text-xl leading-none">close</span>
              </button>
            </div>
            ...
            <div className="aspect-video rounded-lg overflow-hidden mb-4 relative bg-surface-container-high">
              {state.status === 'captcha' && state.imageB64 ? (
                <img src={`data:image/jpeg;base64,${state.imageB64}`} alt="CAPTCHA" className="w-full h-full object-contain" />
              ) : (
            ...
            <button onClick={handleRefreshCaptcha} disabled={state.status === 'verifying'} className="flex items-center space-x-1.5 text-xs font-label text-on-surface-variant/50 hover:text-primary transition-colors mb-5 disabled:opacity-40">
            ...
            <div className="flex items-center space-x-3">
              <button onClick={() => setState({ status: 'idle' })} className="flex-1 py-3 rounded-xl border border-outline-variant/30 text-sm font-label text-on-surface-variant hover:bg-surface-container transition-colors">CANCEL</button>
              <button onClick={handleVerify} disabled={state.status === 'verifying' || !captchaInput.trim()} className="flex-1 py-3 rounded-xl primary-gradient text-on-primary text-sm font-black tracking-widest hover:opacity-90 transition-opacity disabled:opacity-50 flex items-center justify-center gap-2">
                {state.status === 'verifying' && <div className="w-4 h-4 border-2 border-on-primary/30 border-t-on-primary rounded-full animate-spin" />}
                VERIFY
              </button>
            </div>
          </div>
      ...
      {state.status === 'xifan_config' && (
        <XifanDownloadConfigModal
          ...
          onClose={() => setState({ status: 'results', cards: state.cards, keyword: currentKeyword.current })}
