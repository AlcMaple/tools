...
interface HistoryEntry { text: string; time: number }
...
function Settings(): JSX.Element {
  ...
  const [localPath, setLocalPath] = useState('C:/Users/example-user/Documents/BiuProjects/Anime')
  ...
  const { diskFreeLabel, diskFreeRaw, diskTotal, activeTasks, networkOnline, speedLabel } = useSystemStats()
  ...
  const handleSave = (): void => {
    ...
    if (staged.downloadPath !== current.downloadPath) {
      changes.push(staged.downloadPath ? `Download path → ${staged.downloadPath}` : 'Download path cleared')
    }
    if (staged.searchCacheEnabled !== current.searchCacheEnabled) {
      changes.push(`Search cache ${staged.searchCacheEnabled ? 'enabled' : 'disabled'}`)
    }
    ...
    const label = changes.length > 0 ? changes.join('; ') : 'Configuration saved'
    recordChange(label)
    setSaveLabel('saved')
      ...
      <header className="fixed top-0 right-0 left-64 h-16 z-40 bg-[#131313]/80 backdrop-blur-xl border-b border-white/5 flex items-center justify-between px-8">
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="flex items-center gap-2 text-on-surface-variant/60">
            ...
            <span className="font-label text-[10px] tracking-widest">{speedLabel}</span>
          </div>
          <div className="flex items-center gap-1.5">
            <div className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${networkOnline ? 'bg-green-400 animate-pulse' : 'bg-red-500'}`} />
            <span className={`material-symbols-outlined text-sm leading-none ${networkOnline ? 'text-green-400' : 'text-red-500'}`}>
              {networkOnline ? 'wifi_tethering' : 'wifi_off'}
            </span>
            <span className={`font-label text-[10px] tracking-widest ${networkOnline ? 'text-green-400' : 'text-red-500'}`}>
        ...
        <div className="grid grid-cols-12 gap-10 items-start">
          ...
          <section className="col-span-12 lg:col-span-7 space-y-8">
            ...
            <div className="bg-surface-container p-8 rounded-xl border border-white/5">
              <div className="flex items-center justify-between mb-8">
                ...
                <div className="flex items-center gap-2 px-3 py-1 bg-primary/10 rounded-full">
                  ...
                  <span className="font-label text-[10px] text-primary font-bold tracking-widest uppercase">Required</span>
                </div>
              </div>
              <div className="space-y-6">
                <div className="space-y-2">
                  <label className="font-label text-xs uppercase tracking-widest text-on-surface-variant/60">Local Project Path</label>
                  <div className="flex gap-2">
                    <div className="flex-1 bg-surface-container-highest rounded-md px-4 py-3 flex items-center gap-3 focus-within:bg-surface-bright transition-all">
                      <span className="material-symbols-outlined text-on-surface-variant/40 text-sm leading-none">computer</span>
                      <input
                        type="text"
                        value={localPath}
                ...
                <div className="space-y-2">
                  <label className="font-label text-xs uppercase tracking-widest text-on-surface-variant/60">Remote Sync Path</label>
                  <div className="flex gap-2">
                    <div className="flex-1 bg-surface-container-highest rounded-md px-4 py-3 flex items-center gap-3 focus-within:bg-surface-bright transition-all">
                      <span className="material-symbols-outlined text-on-surface-variant/40 text-sm leading-none">cloud_sync</span>
                      <input
                        type="text"
                        value={remotePath}
            ...
            <div className="bg-surface-container p-8 rounded-xl border border-white/5">
              ...
              <div className="space-y-2">
                <label className="font-label text-xs uppercase tracking-widest text-on-surface-variant/60">Save Location</label>
                <div className="flex gap-2">
                  <div className="flex-1 bg-surface-container-highest rounded-md px-4 py-3 flex items-center gap-3">
                    ...
                    <span className={`flex-1 text-sm font-label truncate ${staged.downloadPath ? 'text-on-surface' : 'text-on-surface-variant/30'}`}>
            ...
            <div className="bg-surface-container p-8 rounded-xl border border-white/5">
              ...
              <div className="flex items-start justify-between gap-8 p-6 bg-surface-container-low rounded-lg border border-white/5">
                <div className="space-y-1">
                  <h3 className="font-headline font-bold text-sm uppercase tracking-wider">Enable Search Cache</h3>
                ...
                <label className="relative inline-flex items-center cursor-pointer mt-1 flex-shrink-0">
                  <input
                    ...
                    onChange={(e) => setStaged((s) => ({ ...s, searchCacheEnabled: e.target.checked }))}
                  />
          ...
          <aside className="col-span-12 lg:col-span-5 space-y-6">
            ...
            <div className="bg-surface-variant/70 p-8 rounded-xl border border-white/10 shadow-2xl">
              ...
              <div className="space-y-4 mb-10">
                <div className="flex justify-between items-center">
                  <span className="font-label text-xs text-on-surface-variant/50 uppercase tracking-widest">Platform</span>
                  <span className="font-label text-xs text-on-surface uppercase">{PLATFORM}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="font-label text-xs text-on-surface-variant/50 uppercase tracking-widest">Node ID</span>
                  <span className="font-label text-xs text-on-surface uppercase">{NODE_ID}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="font-label text-xs text-on-surface-variant/50 uppercase tracking-widest">Last Saved</span>
                  <span className="font-label text-xs text-on-surface uppercase">{formatLastSave(lastSaved)}</span>
                </div>
                <div className="w-full h-px bg-white/5 my-2" />
                <div className="flex justify-between items-center">
                  <span className="font-label text-xs text-on-surface-variant/50 uppercase tracking-widest">Storage</span>
                  <span className={`font-label text-xs uppercase ${storageStatus.color}`}>{storageStatus.label}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="font-label text-xs text-on-surface-variant/50 uppercase tracking-widest">Network</span>
                  <span className={`font-label text-xs uppercase flex items-center gap-1.5 ${networkOnline ? 'text-secondary' : 'text-red-400'}`}>
                    <span className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${networkOnline ? 'bg-secondary animate-pulse' : 'bg-red-400'}`} />
                    {networkOnline ? 'Connected' : 'Offline'}
                  </span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="font-label text-xs text-on-surface-variant/50 uppercase tracking-widest">Active Tasks</span>
                  <span className={`font-label text-xs uppercase ${activeTasks > 0 ? 'text-primary' : 'text-on-surface-variant/40'}`}>
                    {activeTasks}
                  </span>
                </div>
                {/* Unsaved changes indicator — always rendered to avoid layout shift */}
                <div className="flex items-center gap-2 pt-2 border-t border-white/5">
                  <span className={`w-1.5 h-1.5 rounded-full flex-shrink-0 transition-colors ${isDirty ? 'bg-yellow-400 animate-pulse' : 'bg-transparent'}`} />
                  <span className={`font-label text-[10px] uppercase tracking-widest transition-opacity duration-200 ${isDirty ? 'text-yellow-400 opacity-100' : 'opacity-0 select-none'}`}>
                    Unsaved changes
                  </span>
                </div>
              </div>
            ...
            <div className="p-8 bg-surface-container rounded-xl border border-white/5">
              <div className="flex items-center gap-3 mb-6">
                ...
                <h2 className="font-headline font-bold text-sm uppercase tracking-widest">Change History</h2>
              </div>
              <ul className="space-y-4">
                {historyEntries.length === 0 ? (
                  ...
                  <li key={i} className="flex items-start gap-4 group">
                    ...
                    <div className="space-y-1">
                      <p className="font-label text-[11px] text-on-surface/80 uppercase leading-tight">{entry.text}</p>
                      <p className="font-label text-[9px] text-on-surface-variant/40 uppercase tracking-tighter">{formatHistoryTime(entry.time)}</p>
                    </div>
                  </li>
                ))}
              </ul>
