...
| `[进行中]` | 正在做（同时最多一条） |
| `[x]` | 已完成 |
| `[等 Win]` | 要 Windows 才能验证（替换成实际平台名） |
| `[阻塞]` | 因其他原因卡住，标签后简短写明原因 |

**为什么用中文文本而不是 emoji**：emoji 没有自解释能力，三个月后回来看
`[▶️]` `[🪟]` `[⛔]` 要查文档；`[进行中]` `[等 Win]` `[阻塞]` 一眼就懂。
`[ ]` 和 `[x]` 保留 —— 通用 markdown checkbox 语法，所有 md 渲染器都认。

每条 todo 是一个功能/任务标题，不展开为多个子条目。需要细节去 📖 详细说明 写。

**在 TODO 段开头加一行图例**（小灰字 blockquote 即可），让文件自解释：
...
现场的最短路径。包括环境、操作步骤、看到的报错原文（**抄字串，不放
截图** —— 字串可 grep，截图不行）。
...
- **目标效果** —— 没有这条就不知道做完什么样算完成
- **复现** —— bug 类条目独有，关键是"现场可重建"
- **已经做过的事** —— 这是 idea log 区别于普通 TODO 列表的关键。它
  不是"接下来怎么做的方案"（那应该在做的时候才决策），而是"已经决策
  完不要再讨论的事"。让下次回来翻 git log 找 commit hash 这种低效活
  能跳过；让下次想出 X 方案的自己先扫一眼"用户已经否过 X 了"。

不写技术实现细节（让代码自己说话）。不预先写"修法思路"（做时再决策）。
...
### 已完成的 TODO 怎么处理
**已完成的条目可以从 TODO 段移除**，不是必须保留为 `[x]`。判断标准：

- 当前阶段还没结束，但某条 todo 完成了 → 视情况决定：
  - 决策密度高（涉及"已经做过的事"段记载的决策） → 留 `[x]` 当锚点
  - 仅是机械工作（一次性脚本、纯 UI 调整） → 直接移除，让 TODO 段保持精简
- 当前阶段完成、整文件归档时 → 进行中的 `[进行中]` 必须改 `[x]`,
  其他 `[x]` 留着作为该阶段的成果清单

核心：TODO 段是「下一步要做的事」，不是「这个阶段做过的所有事」。
已做的事如果有决策价值，进「已经做过的事」段；没有就让它过去。
...
- 在 TODO 标记里用 emoji（`[▶️]` `[🪟]` `[⛔]`）— 不自解释，要查文档；用中文标签
...
- 详细说明里预先写"修法思路 / 根因方向" — 这些是做时才决策的,
  写在这里要么过时要么误导
- 详细说明里嵌截图 — 字串可 grep，截图不行；要截图放 `docs/screenshots/` 然后引用
- 完成的 todo 全部堆在 TODO 段里 — TODO 是"下一步"，已完成的有决策价值就进
  "已经做过的事"段，没价值就移除
...
**已经做过的事**（避免重新踩坑）：
- a1b2c3d 选了原生桥方案 —— Web Audio 在 Android 后台会被挂起,
  必须 MediaSession 保活
...
- e4f5g6h 迁到 CapacitorHttp 走原生网络栈（fetch 在 Android WebView
  里 CORS 行为和 Mac 不一致）
- 当前默认 X-Requested-With 头会被某些 API 拒，已经在 capacitor.config.ts
  里加白名单
