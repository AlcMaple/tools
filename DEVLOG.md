...
3. 若用户恰好在后台校验期间修改追番，当前会话的写操作版本号会拦住更早发出的读取结果，避免刚点击的状态被旧响应短暂盖回。
...
新：本机缓存 ─→ 仅首屏秒开
    服务器数据 ─→ 整份校正页面 + 覆盖缓存
                     └─ 若本页已有写操作：丢弃更早发出的 GET 快照，等写请求收口
```
...
- **页面加载慢的原因还没查明**。冷缓存完整走一遍约 2 分钟，其中 search / watch 每张页 10~30 秒，几乎全部消耗在 TLS 握手失败重试上（`net_error -100`，错误间隔精确 5.002s）；最后一次握手报错后 5~9ms 就判定页面就绪，说明应用侧检测已无延迟。同一 URL 用浏览器直开是秒开，代码里也没有任何网络栈改动（无 `appendSwitch` / `setProxy` / 证书设置）。方向在连接层而非应用逻辑，**不要**再往解析或轮询上猜。
