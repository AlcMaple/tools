...
### 2026-08-07 feat(web): 稀饭账号登录（进行中）

**效果**：

1. 设置页新增稀饭账号模块，支持验证码登录、状态检查和退出登录，登录状态可跨页面同步
2. 服务端按 MapleTools 用户隔离并持久化稀饭 cookie，会话 cookie 加密保存；搜索与播放按登录态走对应会话
3. 未登录或无权限资源返回明确提示，并隔离不同登录用户的播放缓存
