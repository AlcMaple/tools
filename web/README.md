...
加番窗口用「找 BGM 条目 / 自己记一条」页签分开两条路径：手动页会先建立待回填手动卡（负数 `bgmId`），标题只填写一次。卡片出现「回填 BGM」后，可用原卡标题在同一窗口选择 BGM；回填更新官方资料，保留观看进度、自定义标签与用户封面。
...
Node 的 `fetch` 默认不走系统代理。未设置代理环境变量时，开发服务会先尝试直连；如果是
Clash TUN / fake-ip 黑洞，会从 macOS 系统代理、Clash/Mihomo 控制器的混合端口和常见端口
里找本地代理。若 Clash 比服务晚启动，第一次真实请求失败时还会触发一次本地端口重新对齐，
不会定时探测，也不会为重新对齐额外访问源站。

若你的 Clash 是「系统代理模式（非 TUN）」或本地没有可读的控制器端口，跑之前挂上代理环境变量
（换成你 Clash 的 HTTP 端口）：
...
Vercel / 生产 VPS 不走这套本地自动对齐；按部署环境显式配置代理或保持直连。
...
| `DEV_SEARCH_ORIGIN` | 仅本地 dev | 本地没有离线索引时借哪个线上站提供离线结果；用户主动在线搜索仍走本地受控入口 |
