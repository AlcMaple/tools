...
import { GOOGLE_CLIENT_ID,GOOGLE_CLIENT_SECRET,GITHUB_CLIENT_ID,GITHUB_CLIENT_SECRET,LINUXDO_CLIENT_ID,LINUXDO_CLIENT_SECRET } from '../secrets'
...
export function currentGuestKnowledge(){
 return registry.guestSnapshot({enabled:true,permissionVersion:knowledgeHash({modelReady:externalReady(null)}),features:AGENT_FEATURES.filter(f=>(f.id!=='web.github'||Boolean(GITHUB_CLIENT_ID&&GITHUB_CLIENT_SECRET))&&(f.id!=='web.linuxdo'||Boolean(LINUXDO_CLIENT_ID&&LINUXDO_CLIENT_SECRET))).map(f=>f.id),tools:GUEST_DATA_TOOLS,
