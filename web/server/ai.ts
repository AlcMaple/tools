...
function userConfirmedText(m: Material, o: WritingOpts): string {
  ...
  if (m.goodEpisodeNotes && m.goodEpisodeNotes.length) {
    parts.push(
      '用户在这部番的「鉴赏神回」里随手写的备注（只言片语，不是完整点评，只用来帮你把握他的用词和语气；他可能记错或写得很主观，比如把角色性别 / CP 关系搞错，别把里面的错当真）：\n' +
    ...
    [
      { role: 'system', content: systemPrompt(o) },
      {
        ...
        content: [
          ...
          '- 「用户的鉴赏神回备注」只借他的语气和关注点。如果他把人物关系 / 性别记错了，你按正确的写就行，但**不要在选项里特意纠正、也不要加「（其实是男的）」这类括号说明**，正常观众不会这么讲话',
