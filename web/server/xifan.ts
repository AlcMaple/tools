...
xifan.get('/playlist', async (c) => {
  ...
  try {
    ...
    if (e instanceof XifanResolveError) {
      const body = { error: e.message, code: e.code }
      return e.code === 'XIFAN_AUTH_REQUIRED' ? c.json(body, 401) : c.json(body, 403)
    }
    return upstreamFailure(c, e, '稀饭播放页请求失败')
  }
})
...
xifan.get('/resolve', async (c) => {
  ...
  try {
    ...
    return upstreamFailure(c, e, '稀饭线路解析失败')
  }
})
...
xifan.get('/auth/status', async (c) => {
  ...
  try {
    ...
    return upstreamFailure(c, e, '稀饭登录状态校验失败')
  }
})
...
xifan.post('/auth/login', async (c) => {
  ...
  try {
    ...
    return upstreamFailure(c, e, '稀饭登录失败')
  }
})
...
xifan.post('/auth/logout', async (c) => {
  ...
  try {
    ...
    clearXifanResolveCache(session.uid)
    return c.json({ error: e instanceof Error ? e.message : '退出稀饭账号失败' }, 502)
  }
})
...
<style>
  ...
  * { box-sizing: border-box }
  ...
  h1 { font-size: 18px; font-weight: 800; letter-spacing: 0; margin: 0 }
  ...
  #err { display: none; align-items: center; gap: 12px; margin: 0 0 12px; padding: 9px 12px; border-radius: 8px; font-size: 12.5px; font-weight: 600; background: rgba(247,118,142,.12); border: 1px solid rgba(247,118,142,.35); color: #f7768e }
...
<body>
  ...
  <div id="err" role="alert" aria-live="polite"><span id="err-text"></span><a id="auth-link" href="/#/settings/xifan" target="_blank">去登录</a></div>
...
(function(){
  ...
  async function selectLine(source){
    ...
    if (!pl){
      try {
        ...
        var d = await r.json()
        if (!d || d.error || !d.url){
          fail(d && d.error ? d.error : '这条线路解析不到', d && d.code)
          return
        }
  ...
  async function boot(){
    ...
    try {
      ...
      var d = await r.json()
      if (d.error){ fail(d.error, d.code); return }
      lines = d.lines || []
      eps = d.eps || []
