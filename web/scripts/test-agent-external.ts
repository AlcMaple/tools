...
try{
 ...
 await check('本地固定 DeepSeek 自动兼容 Fake-IP，其余端点/生产仍拒绝',()=>{assert(allowedProviderAddress('api.deepseek.com','198.18.4.74',true));assert(!allowedProviderAddress('api.deepseek.com','198.18.4.74',false));assert(!allowedProviderAddress('evil.example','198.18.4.74',true));assert(!allowedProviderAddress('api.deepseek.com','127.0.0.1',true));assert(!allowedProviderAddress('api.deepseek.com','10.0.0.1',true));assert(allowedProviderAddress('api.deepseek.com','1.1.1.1',false))}) // gitleaks:allow -- fixed denial-test addresses; no network request
 ...
 await check('DNS 内网、保留、映射地址被拒；公网地址保留',()=>{for(const ip of ['127.0.0.1','10.0.0.1','172.16.0.1','192.168.1.1','169.254.169.254','100.64.0.1','198.18.1.1','::1','::ffff:127.0.0.1','fc00::1','fe80::1','2001:db8::1'])assert.equal(publicAddress(ip),false,ip);assert(publicAddress('1.1.1.1'));assert(publicAddress('2606:4700::1111'))}) // gitleaks:allow -- fixed denial-test addresses; no network request
