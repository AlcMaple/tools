...
try{
  ...
  await check('用户固定原文和上下文页幅通过后端 revision 更新',async()=>{const c=await controller();c.setDraft('固定原文');await c.send();await fixture.runs.wait(c.getSnapshot().run!.id);await waitFor(()=>c.getSnapshot().connection==='idle');await c.pin(c.getSnapshot().messages[0]);assert.equal(c.getSnapshot().messages[0].pinned,true);await c.tier('64k',false);assert.equal(c.getSnapshot().session?.contextTier,'64k');assert.equal(c.getSnapshot().context?.adaptive,false)})
